...
class Config:
    ...
    OUTPUT_DIR = "/workspace/models/siglip2-jewelry-lora-new"
        ...
        for epoch in range(Config.EPOCHS):
            ...
            for idx, batch in enumerate(pbar):
                ...
                with torch.amp.autocast(self.device) if self.scaler else torch.inference_mode(False):
                    v_emb = self.model.base_model.get_image_features(pixel_values=pixel_values)
                    t_emb = self.model.base_model.get_text_features(input_ids=input_ids, attention_mask=attention_mask)
                    
                    if hasattr(v_emb, 'pooler_output'): v_emb = v_emb.pooler_output
                    if hasattr(t_emb, 'pooler_output'): t_emb = t_emb.pooler_output
